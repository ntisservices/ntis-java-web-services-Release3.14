...
public abstract class AbstractDatexService {
    ...
    public boolean validate(D2LogicalModel request) {
        ...
        if (request.getExchange() != null) {
            LOG.info("Country is {}", request.getExchange().getSupplierIdentification().getCountry().value());
            LOG.info("National Identifier is {}", request.getExchange().getSupplierIdentification().getNationalIdentifier());
        } else {
